...
bool isAccessIndex = true;
bool isAccessVendorsJs= true;
bool isAccessAppCss = true;
bool isAccessAppJs= true;
bool isAccessIcon= true;
bool isAccessRender= true;
bool isAccessNotFound= true;
...
void setupWebserver() {
  ...
  // if (true) {
    ...
    WiFi.begin(getValue("sta_id","Vong Cat-Hide").c_str(), getValue("sta_pass","78787878").c_str());
    while (WiFi.status() != WL_CONNECTED
           && millis() < 30000) {
      delay(100);
      Serial.print(".");
  ...
  WiFi.softAP(getValue("ap_id", "vocauiu2").c_str(), getValue("ap_pass", "12345678").c_str());
